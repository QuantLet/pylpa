...
def residual_bootstrap(yhat, residuals, num_sim=10):
    ...
    # sample = y.copy()
    # yhat, sigsq_hat, eps = garch_forecast(y, sigsq, omega, a, b)
    ...
    boot_sample = np.concatenate([np.array(bootIter()).reshape(-1, 1) for _ in range(num_sim)], axis=1)
...
def get_starting_values(data, model_name, **kwargs):
    if model_name in ['garch', 'arch', 'arma-garch', 'arma-arch']:
        ...
        start_value = [0.01, 0.2, 0.03]  # + [0.2, 0.2, 0.03]
...
def get_mle_estimator(data, start_value, model_name, maxiter, solver=None, **kwargs):
    if model_name in ['garch', 'arch']:
        ...
        MLE = mle_estimator(data, start_value, arma=arma, method=solver, bounds=bounds, options={'maxiter': maxiter})
    else:
        raise NotImplementedError
    return MLE


def get_boot_estimator(data, weights, max_iter, model_name, start_value=None, model_kwargs=None, **kwargs):
    if model_name in ['garch', 'arch', 'arma-garch', 'arma-arch']:
...
def get_sup_estimator(A_data, A_weights, B_data, B_weights, A_mle, B_mle, max_iter, model_name, start_value=None,
